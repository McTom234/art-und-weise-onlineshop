...
class ImagesProductsRepository extends AbstractRepository
{
    ...
    {
        $statement = $this->pdo->prepare("SELECT * FROM `image-product` WHERE product_ID  = :product_ID");
    ...
    public function insertImage($product_ID, $image)
    {
        ...
        $statement = $this->pdo->prepare("INSERT INTO `image-product` (image_ID, product_ID) VALUES (:image_ID, :product_ID)");
        $statement->execute([
            'image_ID' => $image_ID,
